...
- Redis для caching (2-tier + adaptive TTL)
- TailwindCSS для UI

### Функциональность:

- Семантический поиск товаров (не keyword)
- Natural language query understanding (LLM parsing)
- Embeddings для product descriptions
- Vector similarity search в Pinecone
- 2-tier Redis cache (L1: query normalization, L2: results)
- Adaptive TTL strategy (HOT/WARM/COLD)
- Фильтрация по категориям, цене, брендам
- Product recommendations на основе vector similarity

### User Flow:

- Пользователь вводит: "Нужны красные кроссовки для бега до $100"
- AI парсит запрос → brand, color, price, category
- Генерирует embedding через OpenAI
- Ищет в Pinecone по vector similarity с metadata filters
- Возвращает топ-10 товаров с AI explanation
- Cache сохраняет results для повторных запросов
- Показывает "Similar products" recommendations
...
- **Full chat interface** (Vercel AI SDK useChat hook)
...
- Клиент открывает chat widget: "Hi! How can I help?"
- Клиент: "Где мой заказ #12345?"
- AI вызывает function `searchOrder(12345)`
- **Streaming response:** "Your order is... in transit... Expected delivery: Tomorrow"
- Клиент: "Можно изменить адрес?"
- AI понимает контекст (тот же заказ #12345)
- Вызывает function `checkAddressChange(12345)`
- **Streaming response:** "Unfortunately it's too late... Would you like to contact support?"
- Chat history сохраняется в PostgreSQL
...
- **AI генерация с streaming UI** (текст появляется как в ChatGPT)
- **Real-time preview** (streaming text rendering)
- Blog posts & product descriptions generation
- SEO optimization:
  - Meta descriptions
  - Keywords extraction
  - H1/H2 структура
...
- **AI streaming генерирует постепенно:**
  - Title + meta description ← появляется
  - Intro paragraph ← пишется character-by-character
  - 3-4 sections с headings ← streaming animation
  - Conclusion + CTA ← завершает
- Пользователь видит текст появляющийся в реальном времени
- Редактирует параграф 2
